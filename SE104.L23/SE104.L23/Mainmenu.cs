...
namespace SE104.L23
{
    public partial class Mainmenu : Form
    {
        ...
        private void btn_TraCuucd_Click(object sender, EventArgs e)
        {
            ...
            string sql_query = " select BX.TenBenXe as TenBenXeDi, TenBenXeDen, ThoiGianDi, GioDi, SoGheTrong, GiaVe, NgayDi" +
                " from BENXE BX, (select BX1.TenBenXe as TenBenXeDen, ThoiGianDi, GioDi, SoGheTrong, GiaVe, BenXeDi, NgayDi" +
                " from BENXE BX1, (	select CD.ThoiGianDi, CONVERT(date, CD.NgayGio) AS NgayDi, CONVERT(time, CD.NgayGio) AS GioDi, TT.SoGheTrong, DG.GiaVe, T.BenXeDen, T.BenXeDi" +
                " from TUYEN T, CHUYENDI CD, DONGIA DG, TINHTRANGCD TT" +
                " where" + sub_where +
                " and CD.MaTuyen = T.MaTuyen" +
                " and DG.MaTuyen = T.MaTuyen" +
                " and TT.MaChuyenDi = CD.MaChuyenDi" +
                " ) as RES" +
                " where BX1.MaBenXe = RES.BenXeDen) AS RESULT" +
                " where BX.MaBenXe = RESULT.BenXeDi";
            SqlCommand cmd = new SqlCommand(sql_query, con);
            //MessageBox.Show(sql_query);
            using (SqlDataReader reader = cmd.ExecuteReader())
            {
                ...
                while (reader.Read())
                {
                    ...
                    dataGridView2.Rows.Add(count_STT, reader["TenBenXeDen"].ToString(), reader["TenBenXeDi"].ToString(), reader["NgayDi"].ToString().Substring(0, 10), FormatTime(reader["GioDi"].ToString()), reader["GiaVe"].ToString().Substring(0, reader["GiaVe"].ToString().IndexOf(",")), reader["SoGheTrong"].ToString(), (Image)icon);
                    count_STT++;
                }
        ...
        {
            dataGridView4.Rows.Clear();
            MessageBox.Show(dataGridView4.Columns.Count.ToString());
